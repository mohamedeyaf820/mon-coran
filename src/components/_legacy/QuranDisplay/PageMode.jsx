...
export default function PageMode({
  ...
  const canUseQuranComMushaf =
    riwaya === "hafs" &&
    ayahs.some((ayah) =>
      Array.isArray(ayah.words) &&
      ayah.words.some((word) => word.lineNumber && (word.codeV2 || word.codeV1)),
    );

  return (
    <div
      className={`quran-mode-pane quran-mode-pane--page ${
        mushafLayout === "mushaf" && canUseQuranComMushaf
          ? "quran-mode-pane--mushaf-exact"
          : ""
      } ${modePaneShellClass}`}
      role="region"
