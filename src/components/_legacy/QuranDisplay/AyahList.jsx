...
export default function AyahList({
  ...
  const useContinuousFlow = !showWordByWord && !memMode;

  if (useContinuousFlow) {
    return (
      <div
        role="list"
        className={cn(className, "qcom-continuous-list qcom-page-flow")}
        dir="rtl"
        lang="ar"
      >
        {ayahs.map((ayah, index) => {
          ...
          const trans = showTranslation ? getTranslationForAyah?.(ayah) : [];
          const hasTranslation = Array.isArray(trans) && trans.length > 0;
          const showPageSeparator =
            showPageSeparators &&
            (index === 0 || ayahs[index - 1].page !== ayah.page);
          ...
          return (
            <React.Fragment key={ayah.number}>
              ...
              >
                ...
                {hasTranslation ? (
                  <span
                    className="qcom-continuous-translation"
                    dir={lang === "ar" ? "rtl" : "ltr"}
                  >
                    {trans.map((item, transIndex) => (
                      <span key={item.id || item.resourceId || transIndex}>
                        {item.text}
                      </span>
                    ))}
                  </span>
