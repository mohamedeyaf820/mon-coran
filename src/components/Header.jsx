...
export default function Header() {
  ...
  const {
    lang, theme, currentSurah, displayMode, currentPage, currentJuz, riwaya,
    loadedAyahCount, showHome, showDuas,
  } = state;
  ...
  const handleGoTo = (e) => {
    ...
    if (displayMode === "page") {
      if (num >= 1 && num <= 604) {
        ...
        setGoToOpen(false); setGoToValue("");
      }
  ...
  const goToLabel =
    displayMode === "page"
      ? (lang === "fr" ? "Page (1-604)" : lang === "en" ? "Page (1-604)" : "صفحة (-)")
      : displayMode === "juz"
  ...
  const canGoPrev = displayMode === "page" ? currentPage > 1 : displayMode === "juz" ? currentJuz > 1 : currentSurah > 1;
  const canGoNext = displayMode === "page" ? currentPage < 604 : displayMode === "juz" ? currentJuz < 30 : currentSurah < 114;

  const handlePrev = () => {
    set({ showHome: false, showDuas: false });
    if (displayMode === "page") { if (currentPage > 1) set({ currentPage: currentPage - 1, showHome: false, showDuas: false }); }
    else if (displayMode === "juz") { if (currentJuz > 1) dispatch({ type: "NAVIGATE_JUZ", payload: { juz: currentJuz - 1 } }); }
    else { if (currentSurah > 1) dispatch({ type: "NAVIGATE_SURAH", payload: { surah: currentSurah - 1 } }); }
  };

  const handleNext = () => {
    set({ showHome: false, showDuas: false });
    if (displayMode === "page") { if (currentPage < 604) set({ currentPage: currentPage + 1, showHome: false, showDuas: false }); }
    else if (displayMode === "juz") { if (currentJuz < 30) dispatch({ type: "NAVIGATE_JUZ", payload: { juz: currentJuz + 1 } }); }
    ...
    <header className="hdr">
      <div className="hdr__main">
        ...
        <div className="hdr__left">
          <button
            className={cn("hdr__btn", "hdr__btn--menu-main", state.sidebarOpen && "hdr__btn--active")}
            onClick={() => dispatch({ type: "TOGGLE_SIDEBAR" })}
            title={lang === "fr" ? "Menu" : lang === "ar" ? "القائمة" : "Menu"}
            aria-label={lang === "fr" ? "Menu" : lang === "ar" ? "القائمة" : "Menu"}
          >
          ...
          >
            <span className="hdr__brand-mark">
              <PlatformLogo className="w-full h-full" imgClassName="w-full h-full object-cover" decorative />
            </span>
          ...
          <div className="hdr__nav">
            ...
            <Popover open={goToOpen} onOpenChange={setGoToOpen}>
              <PopoverTrigger asChild>
                <button className="hdr__nav-center">
                  ...
                  <span className="hdr__nav-sub">
                    {displayMode === "surah" ? (
                      <>
                        <span>{lang === "ar" ? toAr(currentSurah) : `#${currentSurah}`}</span>
                        <span className="hdr__nav-dot">·</span>
                        <span>{ayahCount}</span>
                      </>
                    ) : displayMode === "page" ? (
                      <>
                        <span>{lang === "ar" ? toAr(currentPage) : currentPage}</span>
                        <span className="hdr__nav-dot">/</span>
              ...
              <PopoverContent
                align="center"
                sideOffset={12}
                className="w-[240px] p-0 border border-[var(--border)] shadow-2xl rounded-2xl bg-[var(--bg-primary)] z-[110]"
              >
        ...
        <div className="hdr__actions">
          ...
          >
            <i className="fas fa-hands-praying" style={{ fontSize: "0.7rem" }} />
          ...
          <div className="hdr__riwaya-toggle" role="group" aria-label="Riwāya">
            <button
              className={cn("hdr__riwaya-btn", riwaya === "hafs" && "hdr__riwaya-btn--active")}
            ...
            <button
              className={cn("hdr__riwaya-btn", riwaya === "warsh" && "hdr__riwaya-btn--active")}
