...
export default function FlashcardsPanel() {
  ...
  const close = () => dispatch({ type: 'SET', payload: { flashcardsOpen: false } });
  ...
  return (
    <div className="modal-overlay" onClick={close} role="dialog" aria-modal="true"
      aria-label={lang === 'fr' ? 'Flashcards vocabulaire' : 'Vocabulary Flashcards'}>
      <div className="modal-panel fc-panel" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="modal-header">
          <div className="modal-title">
            <i className="fas fa-layer-group" />
            {lang === 'fr' ? 'Flashcards' : lang === 'ar' ? 'بطاقات تعليمية' : 'Vocabulary Cards'}
          </div>
        ...
        {done ? (
          <div className="fc-done">
            ...
            <h3>{lang === 'fr' ? 'Session terminée !' : lang === 'ar' ? 'انتهت الجلسة!' : 'Session complete!'}</h3>
            ...
            <div className="fc-done__pct">
              ...
              {lang === 'fr' ? 'de réussite' : lang === 'ar' ? 'نجاح' : 'success rate'}
            </div>
          ...
          <>
            ...
            <div className="fc-count">{idx + 1} / {total}</div>
              ...
              <div className={`fc-card ${flipped ? 'fc-card--flipped' : ''}`}>
                <div className="fc-card__front">
                  ...
                  <div className="fc-card__hint">
                    {lang === 'fr' ? 'Appuyez pour voir' : lang === 'ar' ? 'اضغط للكشف' : 'Tap to reveal'}
                  </div>
                </div>
                <div className="fc-card__back">
                  <div className="fc-card__translation">
                    {lang === 'ar' ? card.en : lang === 'fr' ? card.fr : card.en}
                  </div>
                  <div className="fc-card__root">
                    <span className="fc-card__root-label">{lang === 'fr' ? 'Racine' : 'Root'}</span>
            ...
            {flipped && (
              <div className="fc-actions">
                <button className="fc-btn fc-btn--wrong" onClick={() => answer(false)}>
                  <i className="fas fa-xmark" />
                  {lang === 'fr' ? 'À revoir' : lang === 'ar' ? 'مراجعة' : 'Review'}
                </button>
                <button className="fc-btn fc-btn--correct" onClick={() => answer(true)}>
                  <i className="fas fa-check" />
                  {lang === 'fr' ? 'Connu !' : lang === 'ar' ? 'أعرفه' : 'Got it!'}
                </button>
              </div>
            )}
          </>
