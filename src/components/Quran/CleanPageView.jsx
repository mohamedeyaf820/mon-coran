  ...
  return (
    <div className={`cpv-container${isQCF4 ? " cpv-qcf4" : ""} relative mx-auto w-full max-w-[820px] overflow-hidden rounded-[0.5rem] border border-[var(--cpv-border)] bg-[var(--cpv-bg)] px-12 pt-10 pb-12 shadow-[var(--cpv-shadow)] max-[768px]:rounded-[0.6rem] max-[768px]:px-4 max-[768px]:pt-5 max-[768px]:pb-8 max-[480px]:rounded-[0.5rem] max-[480px]:px-3 max-[480px]:pt-4 max-[480px]:pb-7`}>
      ...
      <div className={getFlowClassName(fontSize, riwaya, isQCF4)} dir="rtl" lang="ar">
        ...
        {ayahs.length > 0 ? <div className="cpv-closure mt-[1.8rem] mb-[0.5rem] flex w-full items-center gap-4" aria-hidden="true"><span className="cpv-closure-line h-px flex-1 bg-[linear-gradient(to_right,transparent,var(--cpv-border))]" /><span className="cpv-closure-ornament shrink-0 whitespace-nowrap font-[var(--font-ui)] text-[0.72rem] font-semibold uppercase tracking-[0.12em] text-[var(--cpv-gold)] opacity-[0.55] max-[768px]:text-[0.66rem]">{lang === "en" ? "End of surah" : "Fin de sourate"}</span><span className="cpv-closure-line h-px flex-1 bg-[linear-gradient(to_left,transparent,var(--cpv-border))]" /></div> : null}
      </div>
