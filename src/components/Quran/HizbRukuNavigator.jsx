...
import { getHizbForAyah, getHizbForPage, getNextSajdah, getPrevSajdah, RUKU_DATA } from "../../data/juz";
...
export function HizbRukuNavigator({
  ...
  const handleHizbClick = () => {
    // Navigate to next hizb
    const nextHizb = currentHizb >= 60 ? 1 : currentHizb + 1;
    // Use the HIZB_DATA to get the surah and ayah for this hizb
    const hizbData = [
    ...
    const hizbInfo = hizbData[nextHizb - 1];
  ...
  return (
    ...
    >
      ...
      >
        <i className="fas fa-bookmark text-[10px] text-primary" />
