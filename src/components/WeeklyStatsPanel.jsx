...
function buildStatsSVG({
  ...
  const totalPages = days.reduce(
    (s, d) => s + (wirdData[d]?.pagesRead || 0),
    0,
  );
  const totalAyahs = days.reduce(
    (s, d) => s + (readingData[d]?.ayahsRead || 0),
    0,
  );
  const activeDays = days.filter(
    (d) => (wirdData[d]?.pagesRead || 0) > 0,
  ).length;
  const completedDays = days.filter((d) => wirdData[d]?.completed).length;

  const safeWeekLabel = (weekLabel || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
export default function WeeklyStatsPanel() {
  ...
  const close = () =>
    dispatch({ type: "SET", payload: { weeklyStatsOpen: false } });
  ...
  useEffect(() => {
    ...
    getWirdHistory(90).then((history) => {
      ...
      try {
        const g = JSON.parse(
          localStorage.getItem("mushaf_khatma_v1") || "null",
        );
          ...
          >
            ...
            <div className="wst-bars !grid !grid-cols-7 !gap-2 !px-3 sm:!px-4">
              {days.map((date, i) => {
                ...
                const maxP = Math.max(
                  1,
                  ...days.map((d) => wirdMap[d]?.pagesRead || 0),
                );
                const pct = Math.round((pages / maxP) * 100);
                const dayLabel = (
                  lang === "fr" ? DAY_LABELS_FR : DAY_LABELS_EN
                )[i];
                return (
                  <div
                    key={date}
