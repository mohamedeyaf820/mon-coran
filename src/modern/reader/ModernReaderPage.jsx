...
async function loadReaderData(route, riwaya, translationLangs, signal) {
  if (route.mode === "surah")
    return getSurahFull(route.value, riwaya, translationLangs, signal);
  if (route.mode === "page")
    return getPageFull(route.value, riwaya, translationLangs, signal);
...
function TajweedText({ text, fallback, enabled }) {
  const segments =
    enabled && text
      ? parseTajweedSegments(text)
      : [{ text: fallback, rule: null }];
  return segments.map((segment, index) => (
    <span
      data-tajwid={segment.rule || undefined}
...
function ModernMushafPage({
  ...
  return (
    <section
      className="modern-mushaf-wrap"
      aria-label={`Page du Coran ${pageNumber}`}
    >
      <article
        className={`modern-mushaf-page modern-mushaf-page--${pageNumber}`}
        dir="rtl"
        lang="ar"
      >
        ...
        <div className="modern-mushaf-page__frame">
          <div className="modern-mushaf-flow">
            {verses.map((verse, index) => {
              ...
              const startsSurah =
                verse.ayahNumber === 1 &&
                (!previous || previous.surahNumber !== verse.surahNumber);
              ...
              return (
                <span className="modern-mushaf-unit" key={verse.key}>
                  ...
                  {startsSurah &&
                    verse.surahNumber !== 1 &&
                    verse.surahNumber !== 9 && (
                      <span className="modern-mushaf-basmala">
                  ...
                  >
                    <TajweedText
                      enabled={showTajweed}
                      fallback={verse.text}
                      text={verse.tajweedText}
                    />
                    <span
                      className="modern-mushaf-ayah__mark"
                      aria-hidden="true"
                    >
                      {verse.ayahNumber}
                    </span>
        ...
        <footer
          className="modern-mushaf-page__number"
      ...
      {selected && (
        <div
          className="modern-mushaf-selection"
          aria-label={`Verset selectionne ${selected.ayahNumber}`}
        >
          <span>
            <strong>{getSurah(selected.surahNumber)?.en}</strong>
            <small>Verset {selected.ayahNumber}</small>
          </span>
          <VerseActions
            bookmarked={bookmarks.has(selected.key)}
            onBookmark={() => onBookmark(selected)}
      ...
      {showTranslation && (
        <div className="modern-mushaf-translations" lang="fr">
          ...
          {verses.map((verse) => (
            <p key={`translation-${verse.key}`}>
              <span>
                {verse.surahNumber}:{verse.ayahNumber}
              </span>
              {verse.translation || "Traduction indisponible."}
            </p>
          ))}
        </div>
      )}
...
function VerseActions({
  ...
  async function shareVerse() {
    ...
    const data = {
      title: `${verse.surahNumber}:${verse.ayahNumber} · Mon Coran`,
      text: `${verse.text}\n${verse.translation}`,
      url,
    };
  ...
  return (
    ...
    >
      ...
      {moreOpen && (
        <div className="modern-verse-more" role="menu">
          <button
            onClick={() => {
              onStart();
              setMoreOpen(false);
          ...
          <button
            ...
            type="button"
          >
            {copied ? <Check size={16} /> : <Copy size={16} />}Copier
          </button>
          <button
            onClick={() => {
              onBookmark();
            ...
            type="button"
          >
            <Bookmark fill={bookmarked ? "currentColor" : "none"} size={16} />
            {bookmarked ? "Retirer le favori" : "Ajouter aux favoris"}
          </button>
          <button
            onClick={() => {
              openNote();
      ...
      {editingNote && (
        ...
        >
          ...
          />
          <div>
            <button onClick={() => setEditingNote(false)} type="button">
              Annuler
            </button>
...
export function ModernReaderPage({ route }) {
  ...
  const [translationLangs, setTranslationLangs] = useState(
    () => settings.translationLangs || [settings.translationLang || "fr"],
  );
  ...
  const [showTajweed, setShowTajweed] = useState(
    () => settings.showTajwid === true,
  );
  const [bookmarks, setBookmarks] = useState(new Set());
  const [reloadKey, setReloadKey] = useState(0);
  const [readingStart, setReadingStartState] = useState(() =>
    getReadingStart(),
  );
  ...
  useEffect(() => {
    const applyPreferences = (event) => {
      ...
      setTranslationLangs(
        next.translationLangs || [next.translationLang || "fr"],
      );
    };
    window.addEventListener("modern-preferences-change", applyPreferences);
    return () =>
      window.removeEventListener("modern-preferences-change", applyPreferences);
  }, []);

  useEffect(() => {
    getAllBookmarks().then((items) =>
      setBookmarks(new Set(items.map((item) => item.id))),
    );
  }, []);
  ...
  useEffect(() => {
    ...
    loadReaderData(route, riwaya, translationLangs, controller.signal)
      .then((data) => {
        ...
        if (first) {
          savePosition(
            first.surahNumber,
            route.ayah || first.ayahNumber,
            first.page || 1,
          );
          addRecentVisit(
            first.surahNumber,
            route.ayah || first.ayahNumber,
            getSurah(first.surahNumber)?.fr,
          );
        }
      })
      .catch((error) => {
        if (error.name !== "AbortError")
          setState({ status: "error", verses: [], error });
      });
    ...
    route.ayah,
    riwaya,
    translationLangs.join(","),
    reloadKey,
  ]);

  useEffect(() => {
    if (state.status !== "ready") return;
    const target = route.ayah
      ? document.getElementById(`ayah-${route.value}-${route.ayah}`)
      : null;
  ...
  useEffect(() => {
    ...
    const target = document.getElementById(
      `ayah-${audio.current.surah}-${audio.current.ayah}`,
    );
  ...
  async function toggleBookmark(verse) {
    ...
    if (bookmarks.has(key))
      await removeBookmark(verse.surahNumber, verse.ayahNumber);
  ...
  async function finishReadingAt(verse) {
    ...
    if (!interval) {
      setReadingFeedback(
        "Choisissez une fin apres le point de depart, dans la meme sourate.",
      );
      return;
    }
    setReadingStartState(null);
    setReadingFeedback(
      `${interval.surah}:${interval.fromAyah} a ${interval.surah}:${interval.toAyah} enregistres`,
    );
  }
  ...
  return (
    <main className="modern-reader" id="modern-main">
      <button
        ...
        className="modern-reader-mobile-options"
      ...
      <section
        className={
          mobileOptionsOpen
            ? "modern-reader-toolbar is-mobile-open"
            : "modern-reader-toolbar"
        }
        aria-label="Navigation de lecture"
      >
        ...
        <div className="modern-reader-picker">
          <a
            ...
            className={route.value <= 1 ? "is-disabled" : ""}
            href={adjacentReaderHref(route, "previous")}
          >
            <ChevronLeft size={19} />
          </a>
          {route.mode === "surah" ? (
            ...
            <label>
              ...
              <input
                max={max}
                min="1"
                onChange={(event) => navigate(route.mode, event.target.value)}
                type="number"
                value={route.value}
              />
            </label>
          ...
          <a
            aria-label="Suivant"
            className={route.value >= max ? "is-disabled" : ""}
            href={adjacentReaderHref(route, "next")}
          >
        ...
        <div className="modern-reader-options">
          <button
            ...
            className={memorizationMode ? "is-active" : ""}
            onClick={() => {
              setMemorizationMode((value) => !value);
              setRevealedVerses(new Set());
            }}
            type="button"
          >
            <Brain size={17} /> Memoriser
          </button>
          <button
          ...
          <button
            ...
            className={showTajweed ? "is-active" : ""}
            disabled={riwaya !== "hafs"}
      ...
      <header className="modern-reader-heading">
        ...
        {route.mode === "surah" && (
          <p className="modern-reader-heading__arabic" lang="ar" dir="rtl">
            {getSurah(route.value)?.ar}
          </p>
        )}
      </header>
      {(readingStart || readingFeedback) && (
        <aside className="modern-reading-session" aria-live="polite">
          <span>
            {readingStart
              ? `Lecture depuis ${readingStart.surah}:${readingStart.ayah}`
              : readingFeedback}
          </span>
          {readingStart && (
            <button
      ...
      {state.status === "loading" && (
        <div className="modern-reader-state" role="status">
          Chargement du texte...
        </div>
      )}
      {state.status === "error" && (
        <div className="modern-reader-state" role="alert">
          ...
          <button onClick={() => setReloadKey((key) => key + 1)} type="button">
            <RotateCcw size={17} /> Reessayer
          </button>
        </div>
      )}
      {state.status === "ready" && !state.verses.length && (
        <div className="modern-reader-state">Aucun verset disponible.</div>
      )}
                ...
                <div key={verse.key}>
                  ...
                  >
                    <div className="modern-reader-verse__meta">
                      <a
                        href={`/surah/${verse.surahNumber}/${verse.ayahNumber}`}
                      >
                        {verse.surahNumber}:{verse.ayahNumber}
                      </a>
                      <VerseActions
                    ...
                    <p
                      className={`modern-reader-verse__arabic${memorizationMode && !revealedVerses.has(verse.key) ? " is-masked" : ""}`}
                      dir="rtl"
                      lang="ar"
                    >
                      <TajweedText
                        ...
                        text={verse.tajweedText}
                      />
                      <span className="modern-ayah-mark" aria-hidden="true">
                        {verse.ayahNumber}
                      </span>
                    </p>
                    {memorizationMode && (
                      <button
                        className="modern-memory-reveal"
                        onClick={() =>
                          setRevealedVerses((current) => {
                            const next = new Set(current);
                            if (next.has(verse.key)) next.delete(verse.key);
                            else next.add(verse.key);
                            return next;
                          })
                        }
                        type="button"
                      >
                        <Eye size={17} />
                        {revealedVerses.has(verse.key) ? "Masquer" : "Reveler"}
                      </button>
                    )}
                    {showTranslation && (
                      <p className="modern-reader-verse__translation" lang="fr">
                        {verse.translation || "Traduction indisponible."}
                      </p>
                    )}
                  </article>
