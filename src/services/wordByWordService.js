...
export async function getWordByWord(surah, ayah, translationLang = 'en') {
  ...
  try {
    ...
    const translationIds = {
      en: 131, // Clear Quran (Mustafa Khattab)
      fr: 136, // Hamidullah
      ar: null, // No translation for Arabic
    };
    
    const wordFields = 'text_uthmani,text_imlaei,transliteration,translation';
    const url = `${BASE_URL}/verses/by_key/${surah}:${ayah}?words=true&word_fields=${wordFields}&translation_fields=text`;
    ...
    try {
      await dbSet(IDB_STORE, cacheKey, { data: words, ts: Date.now() });
    } catch (e) {
