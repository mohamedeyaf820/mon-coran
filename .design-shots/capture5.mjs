...
async function gotoMushaf(p) {
  ...
  await page.waitForSelector('.qc-reader-toolbar__modes button', { timeout: 20000 }).catch(() => console.log('no modes toolbar ' + p));
  const btn = page.locator('.qc-reader-toolbar__modes button', { hasText: 'Mushaf' }).first();
  if (await btn.count() && (await btn.getAttribute('aria-pressed')) === 'false') { await btn.click(); await page.waitForTimeout(2000); }
