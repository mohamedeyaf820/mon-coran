...
const shots = [
  ["vis", "#root ~ .mfp-portal-root .mfp-book .qcm-page-shell { overflow-x: visible !important; overflow-y: visible !important; }"],
  ["pad", "#root ~ .mfp-portal-root .mfp-book .qcm-page-shell { overflow: visible !important; } #root ~ .mfp-portal-root .mfp-book .qcm-lines[data-warsh] { padding-inline: 0.35em; box-sizing: content-box; }"],
];
for (const [name, css] of shots) {
  await page.evaluate((c) => {
    let s = document.getElementById("ab3"); if (!s) { s = document.createElement("style"); s.id = "ab3"; document.head.appendChild(s); }
    s.textContent = c;
  }, css);
  await page.waitForTimeout(600);
  console.log(name, "shell:", await verify());
  await page.screenshot({ path: `.scratch-diag/captures/v2-${name}.png`, clip: { x: 0, y: 68, width: 393, height: 720 } });
}
await browser.close();
