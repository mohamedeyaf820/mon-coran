...
export default function ReaderContextCard({
  ...
  const ar = lang === "ar";
  const totalLabel = ar ? toAr(total) : total;
  const riwayaLabel = ar
    ? t(isWarsh ? "quran.warsh" : "quran.hafs", lang)
    : (isWarsh ? "WARSH" : "HAFS");
  ...
  return (
    ...
    >
        ...
        <div className="reader-context-card__copy min-w-0">
          <div className="reader-context-card__title-row flex items-baseline gap-2 whitespace-nowrap">
            <h2 className="text-xs font-semibold text-[var(--text-secondary)] whitespace-nowrap">
              ...
              <span className="text-[0.72rem] opacity-75 font-normal"> / {totalLabel}</span>
            </h2>
            {secondary ? (
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        <span
          className={`reader-context-card__riwaya inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[0.65rem] font-bold ${ar ? "" : "uppercase tracking-wider"} ${
            isWarsh
