...
// regions: line1 right edge (initial alif), line2 left edge (final noon+mark), line3 left edge (ta marbuta)
const regions = {
...
const states = {
  none: "",
  vis: "#root ~ .mfp-portal-root .mfp-book .qcm-page-shell, #root ~ .mfp-portal-root .mfp-book .qcm-page, #root ~ .mfp-portal-root .mfp-book .qcm-lines, #root ~ .mfp-portal-root .mfp-book .qcm-flow { overflow: visible !important; }",
};
for (const [name, css] of Object.entries(states)) {
  await page.evaluate((c) => {
    let s = document.getElementById("ab4"); if (!s) { s = document.createElement("style"); s.id = "ab4"; document.head.appendChild(s); }
    s.textContent = c;
  }, css);
